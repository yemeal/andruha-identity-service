...
class TestRequestFingerprint:
    def test_mapping_key_order_does_not_change_hash(self) -> None:
        """технический порядок JSON-ключей не влияет на request hash."""
        first = {
            "currency": "RUB",
    ...
    def test_order_items_are_order_insensitive(self) -> None:
        """перестановка позиций не меняет семантику заказа."""
        first = {
            "items": [
    ...
    def test_item_quantity_changes_hash(self) -> None:
        """бизнес-изменение позиции меняет request hash."""
    ...
    def test_unordered_paths_policy_is_explicit(self) -> None:
        """список сортируется только по явной command-specific policy."""
        first = {
            "items": [
    ...
    def test_non_string_mapping_key_is_rejected(self) -> None:
        """canonical JSON не угадывает строковое представление чужого key type."""
        with pytest.raises((TypeError, ValueError), match="string"):
            compute_request_hash({1: "value"})

    def test_naive_datetime_is_rejected(self) -> None:
        """fingerprint не зависит от локального timezone процесса."""
    ...
    def test_non_finite_float_is_rejected(self, value: float) -> None:
        """fingerprint использует переносимый JSON number contract."""
    ...
    def test_equivalent_aware_instants_have_same_hash(self) -> None:
        """один момент времени нормализуется в UTC перед hash."""
...
class TestKeyHash:
    def test_raw_idempotency_key_is_not_used_as_persistent_key(self) -> None:
        """клиентский Idempotency-Key сохраняется как односторонний digest."""
